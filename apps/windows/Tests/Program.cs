...
Check(large.ForegroundWorkers == 12 && large.PrefetchWorkers == 4, "Workstation thumbnail I/O budget");
Check(large.ThumbnailAhead == 8192, "Workstation look-ahead is independent of the 200-photo browsing range");
Check(modest.DecodedBytes < large.DecodedBytes, "Memory-adaptive budget");
Check(large.DecodedBytes == 12L * 1024 * 1024 * 1024, "Workstation prepared-thumbnail cache targets 12 GiB");
...
NativeScrollPrecisionTests.Run(Check);
NativeWheelTargetTests.Run(Check);
BulkCollectionTests.Run(Check);
BrowsingPolicyTests.Run(Check);
await SharedThumbnailPreparationTests.RunAsync(Check);
await ThumbnailWorkGateTests.RunAsync(Check);
ThumbnailKeyCacheTests.Run(Check);
await CompactCatalogTests.RunAsync(Check);
await GalleryFeatureTests.RunAsync(Check);
CatalogMemoryTests.Run(Check);
await GalleryWorkPolicyTests.RunAsync(Check);
