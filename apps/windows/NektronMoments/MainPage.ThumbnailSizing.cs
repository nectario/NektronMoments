...
public sealed partial class MainPage
{
    ...
    private void BeginThumbnailSizing()
    {
        ...
        _sizingRoot = compositor.CreateContainerVisual();
        _sizingRoot.Size = new Vector2((float)Gallery.ActualWidth, (float)Gallery.ActualHeight);
        _sizingRoot.Clip = compositor.CreateInsetClip();
        var maxColumns = Math.Max(1, (int)((Gallery.ActualWidth - 16) / 112));
        ...
        var movement = compositor.CreateVector3KeyFrameAnimation(); movement.Target = "Offset";
        movement.InsertExpressionKeyFrame(1, "this.FinalValue"); movement.Duration = TimeSpan.FromMilliseconds(90);
        var sizeAnimation = compositor.CreateVector2KeyFrameAnimation(); sizeAnimation.Target = "Size";
        sizeAnimation.InsertExpressionKeyFrame(1, "this.FinalValue"); sizeAnimation.Duration = TimeSpan.FromMilliseconds(70);
        var motion = compositor.CreateImplicitAnimationCollection(); motion["Offset"] = movement; motion["Size"] = sizeAnimation;
        for (var index = first; index < last; index++) {
            ...
            var shape = compositor.CreateRoundedRectangleGeometry(); shape.CornerRadius = new Vector2(10);
            tile.Clip = compositor.CreateGeometricClip(shape);
            var back = compositor.CreateSpriteVisual(); back.Brush = compositor.CreateColorBrush(background);
            var imageSource = compositor.CreateVisualSurface();
            ...
            var imageBrush = compositor.CreateSurfaceBrush(imageSource); imageBrush.Stretch = CompositionStretch.UniformToFill;
            var image = compositor.CreateSpriteVisual(); image.Brush = imageBrush;
            var captionSource = compositor.CreateVisualSurface();
            ...
            var captionBrush = compositor.CreateSurfaceBrush(captionSource);
            ...
            var caption = compositor.CreateSpriteVisual(); caption.Brush = captionBrush;
    ...
    private void ApplySizingFrame(bool force)
    {
        ...
        if (!force && (now - _lastSizingFrame) * 1000d / Stopwatch.Frequency < 16) return;
        ...
        var columns = Math.Max(1, (int)((Gallery.ActualWidth - Gallery.Padding.Left - Gallery.Padding.Right - 8) / width));
    ...
    private void CommitThumbnailSizing()
    {
        ...
        try {
            ...
            _sizingRoot?.Dispose(); _sizingRoot = null;
            foreach (var tile in _sizingTiles) {
                tile.ImageSource.Dispose(); tile.CaptionSource.Dispose(); tile.Root.Dispose();
                tile.Back.Dispose(); tile.Image.Dispose(); tile.Caption.Dispose(); tile.Shape.Dispose();
            }
            _sizingTiles.Clear(); _thumbnailLayoutCommit = false;
    ...
    private async Task RestoreThumbnailLookaheadAsync(int generation)
    {
        try {
            ...
            if (!_isThumbnailSizing && generation == _sizingGeneration) { ResizeGallery(); ScheduleThumbnailWarm(); _ = PrimeBufferAsync(); }
